...
train_dir = os.path.join("data", "train_steve_dummyc")
val_dir = os.path.join("data", "val_steve_dummyc")
...
def main(rate):
    ...
    # Train the model
    model_ft, loss_dict, acc_dict, best_acc = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler, dataloaders,
                                                dataset_sizes, num_epochs=NUM_EPOCHS)

    # Save the trained model
    torch.save(model_ft.state_dict(), "trained_model_" + str(rate/100) + ".pt")

    # Basic visualizations of the model performance
    fig = plt.figure(figsize=(20, 10))
    plt.title("Train - Validation Loss for " + str(rate/100))
    plt.plot(loss_dict['train'], label='train')
    plt.plot(loss_dict['val'], label='validation')
    plt.xlabel('num_epochs', fontsize=12)
    plt.ylabel('loss', fontsize=12)
    plt.legend(loc='best')
    plt.savefig('graphs/train_val_loss_plot_' + str(rate/100) + '.png')

    fig = plt.figure(figsize=(20, 10))
    plt.title("Train - Validation ACC for " + str(rate/100))
    plt.plot(acc_dict['train'], label='train')
    plt.plot(acc_dict['val'], label='validation')
    plt.xlabel('num_epochs', fontsize=12)
    plt.ylabel('ACC', fontsize=12)
    plt.legend(loc='best')
    plt.savefig('graphs/train_val_acc_plot_' + str(rate/100) + '.png')

    rate_stats = {'best acc': best_acc, 'best loss': loss_dict['val'][:-1]}
  #  stats_name = 'Model results for ' + str(rate) + ' val-to-train dist'
    with open('Rate_Choice_Results.txt', 'a') as f:
        f.write(f"Results for rate {rate} are: best acc: {best_acc}, best loss: {loss_dict['val'][:-1]} \n")
